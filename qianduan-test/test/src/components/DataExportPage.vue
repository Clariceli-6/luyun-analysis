<template>
    <div class="app-container">
      ...
      <div class="main-content">
        ...
        <main class="content">
          <!-- 查询模块 -->
          <div class="module" v-if="activeMenu === 'export'">
          ...
          <!-- 查询结果模块：设备信息查询 -->
          <div class="module" v-if="activeMenu === 'export' && isDeviceSearchActive">
            <table class="data-table">
              <thead>
                <tr>
                  <th v-for="(item, index) in columns" :key="index">
                    <input type="checkbox" v-model="selectedColumns" :value="item.value">
                    {{ item.label }}
                  </th>
                </tr>
              </thead>
              ...
              <tr v-for="item in paginatedData" :key="item.id">
                <td v-for="col in columns" :key="col.value">
                  <span v-if="selectedColumns.includes(col.value)">
                      {{ isTimestamp(col.value) ? formatTimestamp(item[col.value]) : item[col.value] }}
                  </span>
          ...
          <!-- 查询结果模块：检测对象信息查询 -->
          <div class="module" v-if="activeMenu === 'export' && isTitleSearchActive">
            <table class="data-table">
              <thead>
                <tr>
                  <th v-for="(item, index) in titleColumns" :key="index">
                    <input type="checkbox" v-model="selectedTitleColumns" :value="item.value">
                    {{ item.label }}
                  </th>
                </tr>
              </thead>
              <tbody>
              <tr v-for="item in paginatedData" :key="item.id">
                <td v-for="col in titleColumns" :key="col.value">
                    <span v-if="selectedTitleColumns.includes(col.value)">
                      {{
                        col.value === 'longitude'
                            ? convertLongitude(item[col.value])
                            : col.value === 'latitude'
                                ? convertLatitude(item[col.value])
                                : item[col.value]
                      }}
                    </span>
                  <span v-else>--</span> <!-- 占位符 -->
...
export default {
  data() {
    return {
      activeMenu: 'export', // 这里应该默认为export而不是search
      searchKeyword: '',
      searchTitle: '',
      currentPage: 1,
      pageSize: 6,
      ...
      columns: [
        { label: '感知设备编号', value: 'deviceId' },
        { label: '设备类别', value: 'deviceType' },
        { label: '输出时间', value: 'timestampOfDevOut' },
        { label: '计算应用时间', value: 'timestampOfDetIn' },
        { label: '输出结果时间', value: 'timestampOfDetOut' },
        { label: '感知对象数量', value: 'objectiveNum' },
      ],
      titleColumns: [
      ...
      selectedColumns: ['deviceId', 'deviceType', 'timestampOfDevOut', 'timestampOfDetIn', 'timestampOfDetOut', 'objectiveNum'],
      selectedTitleColumns: ['deviceId', 'type', 'longitude', 'latitude', 'speed'],
    };
  },
  created() {
  ...
  methods: {
    ...
    isTimestamp(colName) {
    return ['timestampOfDevOut', 'timestampOfDetIn', 'timestampOfDetOut'].includes(colName);
  },


    async searchData() {
  const fieldBooleanMap = {
    deviceType: 'is_device_type',
    timestampOfDevOut: 'is_timestamp_of_dev_out',
    timestampOfDetIn: 'is_timestamp_of_det_in',
    timestampOfDetOut: 'is_timestamp_of_det_out',
    objectiveNum: 'is_objective_num',
  };

  // 初始化所有布尔字段为 false
  const allBooleanFields = {
    is_device_type: false,
    is_timestamp_of_dev_out: false,
    is_timestamp_of_det_in: false,
    is_timestamp_of_det_out: false,
    is_objective_num: false,
  };

  // 将选中的字段标记为 true
  this.selectedColumns.forEach(col => {
    const flagKey = fieldBooleanMap[col];
    if (flagKey) {
      allBooleanFields[flagKey] = true;
    }
  });
  ...
  try {
    ...
    const response = await axios.post('/excel/deviceId', {
      device_id: this.searchKeyword,
      page: this.currentPage,
      page_size: this.pageSize,
      ...allBooleanFields, // ✅ 拆成多个字段传递，但不是一个整体对象
    },
    {
      headers: {
        'token': token,
...
async searchByTitle() {
  ...
  // 如果是设备信息查询
  if (this.isDeviceSearchActive) {
    const response = await axios.get('/excel/exportDevice', {
      params: {
        deviceId: this.searchKeyword, // 将参数传递给 params
      },
      headers: {
        'token': token,
      },
    ...
    if (Array.isArray(result)) {
      selectedData = result.map(item => {
        const data = {};
        // 遍历勾选的列
        this.columns.forEach(colObj => {
          const col = colObj.value;
          if (this.selectedColumns.includes(col)) {
            switch (col) {
              case 'deviceId':
                data[col] = item.device_id;
                break;
              case 'deviceType':
                data[col] = item.device_type;
                break;
              case 'timestampOfDevOut':
                data[col] = item.timestamp_of_dev_out;
                break;
              case 'timestampOfDetIn':
                data[col] = item.timestamp_of_det_in;
                break;
              case 'timestampOfDetOut':
                data[col] = item.timestamp_of_det_out;
                break;
              case 'objectiveNum':
                data[col] = item.objective_num;
                break;
              default:
                data[col] = '';
            }
  ...
  // 如果是检测对象信息查询
    ...
    const response = await axios.get('/excel/exportTime', {
      params: {
    ...
    if (Array.isArray(result)) {
      selectedData = result.map(item => {
        const data = {};
        // 遍历勾选的列
        this.titleColumns.forEach(colObj => {
          const col = colObj.value;
          if(this.selectedTitleColumns.includes(col)) {
            // 映射字段名
            switch (col) {
              case 'deviceId':
                data[col] = item.device_id;
              ...
              case 'type':
                ...
                break;
              case 'longitude':
                data[col] = item.longitude;
                break;
              case 'latitude':
                data[col] = item.latitude;
                break;
              case 'speed':
                data[col] = item.speed;
                break;
              default:
                data[col] = '';  // 如果没有匹配的字段名，设置为空
            }
          }
        });
        return data;
